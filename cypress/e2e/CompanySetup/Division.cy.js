describe('Division Tests', () => {
    ...
    it('should create New Item', () => {
        ...
        cy.get('[routerlink="/master/division"]').should('be.visible').click();
        cy.get('.border-l > .p-ripple').click();
        cy.get('#Name').type('ACCOUNT SUB');
        cy.get('#Tax > .p-select-dropdown').should('be.visible').click();
        cy.get('.p-iconfield > .p-inputtext').should('be.visible').type('ACCOU');
        cy.get('#Tax_1').should('be.visible').click();
        //To save New Tax
        // cy.get('.p-ripple.ng-star-inserted > .p-ripple').should('be.visible').click();

    });
    it('should Edit Division', () => {
        cy.get('#pn_id_1_tab_1 > .pi').should('be.visible').click();
        cy.get('[routerlink="/master/division"]').should('be.visible').click();
        cy.get(':nth-child(2) > .text-right > .ml-2').should('be.visible').click();
        cy.wait(2000);
        cy.get('#Name').should('be.visible').clear().type(' New ');
        //Update division Name
        cy.get('.gap-2 > .btn-new > .p-ripple').click();

    });
    it('should Add sub Division', () => {
        cy.get('#pn_id_1_tab_1 > .pi').should('be.visible').click();
        cy.get('[routerlink="/master/division"]').should('be.visible').click();
        cy.get(':nth-child(16) > .flex > .text-sm').should('be.visible').click();
        cy.get('.p-button-success').click();
        cy.get('.p-popover-content > :nth-child(2)').click();
        cy.get('#Name').type('Account Dubai');
        cy.get('.p-ripple.ng-star-inserted > .p-ripple').click();

    });
    it.only('should Filter Work in Properly', () => {
        cy.get('#pn_id_1_tab_1 > .pi').should('be.visible').click();
        cy.get('[routerlink="/master/division"]').should('be.visible').click();
